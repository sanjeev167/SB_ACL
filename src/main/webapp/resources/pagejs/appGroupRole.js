...
$(document).ready(function() {
	...
	$(document).on("click", "#saveFormButtonId", function(e) {
		...
		cleanAllMsg();
		var appGroupRoleNameUniqueness="Yes";
		saveAndUpdateRecord(appGroupRoleNameUniqueness);		
	...
	$(document).on("click", "#updateFormButtonId", function() {
		cleanAllMsg();
		var appGroupRoleNameUniqueness="No";
			saveAndUpdateRecord(appGroupRoleNameUniqueness);	
	...
	$(document).on("click", "#searchFormButtonId", function(e) {
		...
		searchAndReloadGrid(departmentNameId,groupNameId,roleNameId);
	});
...
function reloadGrid() {
	...
	window.location.replace("/sec/roleToGroup/");
...
function searchAndReloadGrid(departmentNameId,groupNameId,roleNameId) {
	clearCheckBoxSelection();
	var table = $('#appGroupRoleId').DataTable();
	table.ajax.url(
			"paginated?departmentNameId=" + departmentNameId + "&groupNameId=" +groupNameId+"&roleNameId="+roleNameId).load();
...
var t;
function loadGrid(departmentNameId,groupNameId,roleNameId) {
	t = $('#appGroupRoleId').DataTable(
			{
				"responsive": true,
				...
				"ajax" : {					
					"url" : "paginated?departmentNameId=" + departmentNameId+ "&groupNameId=" + groupNameId + "&roleNameId=" + roleNameId + "",
					"type" : "POST",
				},
...
var baseUrl = '/sec/roleToGroup';

function clearFormData() {	
	$('#groupNameId').empty().append('<option selected="selected" value="">-Select-</option>');
	$('#roleNameId').empty().append('<option selected="selected" value="">-Select-</option>');
...
function loadRecordInForm(response) {
	$('#appGroupRoleRecordId').val(response.id);	
	...
	$('#roleNameId').empty().append('<option  value="">-Select-</option>');	
	selectedId=response.roleNameId;
	loadRoleCombo(response.departmentNameId,selectedId);	
}
...
function saveAndUpdateRecord(appGroupRoleNameUniqueness) { 
	
	/* stop form from submitting normally */
	event.preventDefault();
	/* get the form data of the from */	
	var id = $('#appGroupRoleRecordId').val();	
	var departmentNameId = $('#departmentNameId').val();	
	var groupNameId = $('#groupNameId').val();
	var roleNameId = $('#roleNameId').val();
	...
	var json = {
			...
			"roleNameId" : roleNameId,"appGroupRoleNameUniqueness":appGroupRoleNameUniqueness
	};	
	$.ajax({
		type : 'POST',
...
function loadDepartmentCombo(){	
	/* stop form from submitting normally */	
	method = 'GET';
	url = "/sec/roleToGroup/department" + "/list";	
	$
	.ajax({
		type : method,
		url : url,
...
function loadGroupCombo(id,selectedId){	
	...
	/* stop form from submitting normally */	
	method = 'GET';
	url = "/sec/roleToGroup/group" + "/list?id=" + id;	
	//alert("url = "+url);
	$
	.ajax({
		type : method,
...
function loadRoleCombo(id,selectedId){		
	/* stop form from submitting normally */	
	method = 'GET';
	url = "/sec/roleToGroup/role" + "/list?id=" + id;
	$.ajax({
		type : method,
		url : url,
		data : {},
