...
function reloadGrid() {
	...
	window.location.replace("/sec/adminUser/");
...
function searchAndReloadGrid(departmentNameId,categoryNameId,userLoginId) {
	clearCheckBoxSelection();
	var table = $('#appAdminUserId').DataTable();
	table.ajax.url(
	...
	t = $('#appAdminUserId').DataTable(
			{
				...
				"ordering" : true,
				"searching" : true,
				"aaSorting" : [ [ 3, "asc" ], [ 4, "asc" ],[ 5, "asc" ],[ 6, "asc" ] ],
				"ajax" : {
				...
				"columns" : [
					...
					},
					{
						...
						"name" : "departmentName",
						"title" : "App. Context",
						//"searchable" : true,//It needs to be set here when cell specific search needs to be disable
						//"orderable" : true,
					},
					{
						"data" : "categoryName",
...
function loadRecordInForm(response) {
	$('#appAdminUserRecordId').val(response.id);	
...
function saveAndUpdateRecord() { 
	...
	var id = $('#appAdminUserRecordId').val();	
	var departmentNameId = $('#departmentNameId').val();	
	var categoryNameId = $('#categoryNameId').val();
	var name = $('#nameId').val();
...
function loadDepartmentCombo(){	
	/* stop form from submitting normally */	
	method = 'GET';
	url = "/sec/adminUser/department" + "/list";	
	$
	.ajax({
		type : method,
		url : url,
