...
$(document).ready(function() {
	...
	$(document).on("click", "#searchFormButtonId", function(e) {
		...
		searchAndReloadGrid(department);
	});
...
function reloadGrid() {
	...
	window.location.replace("/sec/env/department/");
...
function searchAndReloadGrid(departmentName) {
	clearCheckBoxSelection();
	var table = $('#departmentId').DataTable();
	table.ajax.url(
			"paginated?departmentName=" + departmentName + "").load();
...
function GetSelectedRowID() {
	...
	$(".chkIndvRow").each(function() {
		if ($(this).is(':checked')) {
			...
			checkedRowNames.push(selectedRows[0].departmentName + "</br>");
...
var t;
function loadGrid(departmentName) {
	
	t = $('#departmentId').DataTable(
			{
				//"colReorder": true,
				...
				"aaSorting" : [ [ 3, "asc" ]],
				"ajax" : {
					"url" : "paginated?departmentName=" + departmentName + "",
...
var baseUrl = '/sec/env/department';
...
function loadRecordInForm(response) {
	$('#departmentRecordId').val(response.id);
	$('#departmentNameId').val(response.departmentName);
...
function showBusinessEerror(fieldErrMsgMap) {
	$('#departmentNameId_err').html(fieldErrMsgMap.departmentName);	
}
...
function saveAndUpdateRecord() { 
	...
	var id = $('#departmentRecordId').val();
	var departmentName = $('#departmentNameId').val();
