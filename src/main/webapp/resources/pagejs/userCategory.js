...
$(document).ready(function() {
	...
	$(document).on("click", "#saveFormButtonId", function(e) {
		cleanAllMsg();
		var userCategoryNameUniqueness="Yes";
		saveAndUpdateRecord(userCategoryNameUniqueness);		
	...
	$(document).on("click", "#updateFormButtonId", function() {
		cleanAllMsg();
		var userCategoryNameUniqueness="No";
		saveAndUpdateRecord(userCategoryNameUniqueness);	
	...
	$(document).on("click", "#searchFormButtonId", function(e) {		
		...
		searchAndReloadGrid(categoryName, departmentNameId);//While searching countryId will be passed not the name
	});
...
function reloadGrid() {
	...
	window.location.replace("/sec/userCategory/");
...
function searchAndReloadGrid(categoryName,departmentNameId) {
	clearCheckBoxSelection();
	var table = $('#userCategoryId').DataTable();
	table.ajax.url(
			"paginated?categoryName=" + categoryName + "&departmentNameId="+departmentNameId+"").load();
...
function GetSelectedRowID() {
	...
	$(".chkIndvRow").each(function() {
		if ($(this).is(':checked')) {
			...
			checkedRowNames.push(selectedRows[0].departmentName +" => "+selectedRows[0].categoryName + "</br>");
...
var t;
function loadGrid(categoryName, departmentNameId) {
	t = $('#userCategoryId').DataTable(
			{
				//"colReorder": true,
				...
				"ajax" : {
					"url" : "paginated?categoryName=" + categoryName + "&departmentNameId=" + departmentNameId  + "",
...
function applyReadOnlyProp() {
	$('#categoryNameId').prop("readonly", true);
...
function removeReadOnlyProp() {
	$('#categoryNameId').prop("readonly", false);
...
function loadRecordInForm(response) {
	$('#categoryNameRecordId').val(response.id);	
	$('#departmentNameId').val(response.departmentNameId);
	$('#categoryNameId').val(response.categoryName);
}
...
function showBusinessEerror(fieldErrMsgMap) {
	$('#categoryName_err').html(fieldErrMsgMap.categoryName);
	$('#departmentNameId_err').html(fieldErrMsgMap.departmentNameId);	
}

function saveAndUpdateRecord(userCategoryNameUniqueness) { 
	
	/* stop form from submitting normally */
	event.preventDefault();
	/* get the form data of the from */
	// var formSerializeData = $('#commonFormId').serialize();
	var id = $('#categoryNameRecordId').val();
	var categoryName = $('#categoryNameId').val();
	var departmentNameId = $('#departmentNameId').val();	
	...
	var json = {
			...
			"departmentNameId" : departmentNameId,"userCategoryNameUniqueness"	:userCategoryNameUniqueness		
	};
...
function loadDepartmentCombo(){	
	/* stop form from submitting normally */	
	method = 'GET';
	url = "/sec/userCategory/department" + "/list";	
	$
	.ajax({
		type : method,
		url : url,
