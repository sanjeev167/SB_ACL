...
$(document).ready(function() {
	...
	$(document).on("click",".addClass",function() {
		...
		$("#commonModalTitleId").html('Add a group');
	...
	$(document).on("click", "#saveFormButtonId", function(e) {
		cleanAllMsg();
		var appGroupUniqueness="Yes";
		saveAndUpdateRecord(appGroupUniqueness);		
	...
	$(document).on("click", "#searchFormButtonId", function(e) {
		...
		searchAndReloadGrid(groupName, departmentNameId);//While searching countryId will be passed not the name
	});
...
function reloadGrid() {
	...
	window.location.replace("/sec/group/");
...
function searchAndReloadGrid(groupName,departmentNameId) {
	clearCheckBoxSelection();
	var table = $('#appGroupId').DataTable();
	table.ajax.url(
			"paginated?groupName=" + groupName + "&departmentNameId="+departmentNameId+"").load();
...
function GetSelectedRowID() {
	...
	$(".chkIndvRow").each(function() {
		if ($(this).is(':checked')) {
			...
			checkedRowNames.push(selectedRows[0].departmentName +" => "+selectedRows[0].groupName + "</br>");
...
var t;
function loadGrid(groupName, departmentNameId) {
	t = $('#appGroupId').DataTable(
			{
				//"colReorder": true,
				...
				"ajax" : {
					"url" : "paginated?groupName=" + groupName + "&departmentNameId=" + departmentNameId  + "",
...
function applyReadOnlyProp() {
	$('#groupNameId').prop("readonly", true);
...
function removeReadOnlyProp() {
	$('#groupNameId').prop("readonly", false);
...
function loadRecordInForm(response) {
	$('#groupRecordId').val(response.id);	
	$('#departmentNameId').val(response.departmentNameId);
	$('#groupNameId').val(response.groupName);
}
...
function saveAndUpdateRecord(appGroupNameUniqueness) { 
	...
	var json = {
			...
			"appGroupNameUniqueness":appGroupNameUniqueness
	};
