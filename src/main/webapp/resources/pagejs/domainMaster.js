...
function reloadGrid() {
	...
	window.location.replace("/adm/pvt/sec/acl/dmnMaster/");
...
var baseUrl = '/adm/pvt/sec/acl/dmnMaster';
...
function loadDepartmentCombo(){	
	/* stop form from submitting normally */	
	method = 'GET';
	url = "/adm/pvt/sec/env/department" + "/list";	
	$
	.ajax({
		type : method,
		url : url,
...
function loadModuleCombo(id){	
	...
	/* stop form from submitting normally */	
	method = 'GET';
	url = "/adm/pvt/sec/env/module" + "/list?id=" + id;		
	$
	.ajax({
		type : method,
		url : url,
