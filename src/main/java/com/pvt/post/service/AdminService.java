...
import com.pvt.post.domain.AdminPost;
import com.pvt.post.domain.Post;

/**
 * Service for processing Admin related posts. 
 * <p>
 * For a complete reference to Spring JDBC and getJdbcTemplate()
...
public class AdminService extends JdbcDaoSupport implements GenericService {
	...
	public List<Post> getAll() {
		...
		// Run query then return result
		List<Post> posts=getJdbcTemplate().query(sql, mapper);
	...
	public Boolean add(Post post) {
		try {
			...
			SimpleJdbcInsert simpleJdbcInsert = new SimpleJdbcInsert(dataSource).withTableName("admin_post")
	...
	public Boolean edit(Post post) {
		try {
			...
			String updateSql = "update admin_post set date = ?, " + "message = ? where id = ?";
